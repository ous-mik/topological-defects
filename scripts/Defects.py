...
start_frame = 0
close_enough = 50  # increased from 40 to 50
scale = 35
...
def do_it_all(path_inn, path_out, f, ps_x, ps_y, window_size, overlap, search_area_size):
    ...
    names = names[start_frame:]  # setting the range of frames
    ...
    for frame_a, frame_b in zip(names, names[1:]):  # Calculate PIV
        ...
        if np.isnan(u).any():  # Some director fields may contain NaN values. If this happens: notify
            ...
            # Plotting data
            fig, ax = plt.subplots()
            ax.quiver(rescale_x, rescale_y, rescale_u / r, rescale_v / r,
                pivot="mid",
                scale=scale,
                color = "k",
                alpha=1,
                width=0.0036)
            ax.scatter(k2_list_positive, k1_list_positive, color="C1", marker="D", edgecolors="k", s=40)
            ax.scatter(k2_list_negative, k1_list_negative, color="C4", edgecolors="k", s=40)
            #ax.imshow(mag, extent=(0, ps_x, 0, ps_y), cmap="Blues_r", alpha=1, vmin=0, vmax=4)
            p = ax.imshow(im, extent=(0, ps_x, 0, ps_y), vmin=1000, vmax=30000, origin="lower", cmap="Greens", alpha=0.5)
            # ax.streamplot(x, y, u, v, 14, linewidth=1, color="k", maxlength=4.0, minlength=0.02)
            for art in ax.get_children():
                if not isinstance(art, matplotlib.patches.FancyArrowPatch):
                    continue
                art.remove()
            plt.colorbar(p, label="nuclear fluorescence")
            plt.gca().set_aspect('equal', adjustable='box')
            plt.tight_layout()
            plt.savefig(path_out + frame_a, dpi=300)
            plt.close()
